...
**RAG 推理优化（2025-2026）**：RAG 的一个性能瓶颈是每次检索后都要重新做 pre-filling（把检索到的文档重新编码进 KV Cache）。新工作致力于复用文档的 KV Cache：
- **CacheFocus**：动态缓存重定位，离线缓存查询无关的文档 KV，在线时通过缓存重定位和层自适应剪枝加速。
- **Cache-Craft**：管理 chunk-level KV Cache，识别可复用的 chunk 缓存，通过少量重计算保持输出质量。
- **RAGCache**：将检索知识的中间状态组织为知识树，在 GPU 和主机内存中缓存，基于访问频率、大小和成本的替换策略。
...
**2026 年新方向**：KV Cache 复用正在重塑 RAG 的推理成本结构——从"每次重新编码检索文档"转向"缓存文档的 KV 状态并智能复用"。同时，TeleRAG（MLSys 2026）引入前瞻检索（lookahead retrieval），在生成过程中预测下一步需要的信息并提前检索，减少生成-检索的串行等待。
