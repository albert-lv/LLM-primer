...
2026 年的新进展是 test-time compute 从"花更多"走向"分得巧"：Adaptive Parallel Reasoning（[[ref:berkeley2026-parallel-reasoning]]）等工作用强化学习让模型自己学会按问题难度分配推理计算——简单问题走短串行推理，复杂问题自动展开多个并行推理线程再汇总收敛，突破了串行 CoT 的长度瓶颈。这可以看作解码策略的自然延伸：解码不再只是"怎么选下一个 token"，而是"整个推理过程投入多少计算、以什么拓扑展开"。
...
<OpenQuestions questions={[
  ...
  { q: '自适应并行推理中，并行线程数与串行深度的最优配比如何随任务类型变化？RL 学到的计算分配策略能否跨领域泛化？', papers: ["berkeley2026-parallel-reasoning", "snell2024-test-time-compute"] },
]} />
</TierBlock>
